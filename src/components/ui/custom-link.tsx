...
export default function CustomLink({ size = 'md', className = '', children, href, as = 'a', ...props }: CustomLinkProps) {
  ...
  const commonClasses = `link-research inline-flex items-center ${textSize} font-semibold group ${className}`;
  const content = (
    <>
      <span>{children}</span>
      <ArrowRight className={`${iconSize} ml-2 group-hover:translate-x-1 transition-transform shrink-0`} />
    </>
  );
